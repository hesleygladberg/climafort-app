...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        # Click the Entrar (login) button to submit the login form
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Login Successful - Dashboard Loaded').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: The login was not successful, and the dashboard page did not display as expected.')
